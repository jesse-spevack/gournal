...
class HabitTrackingCacheTest < ActionDispatch::IntegrationTest
  ...
  test "cache invalidation works with 304 responses for unchanged data" do
    habit = Habit.create!(
      user: @user,
      name: "Morning Exercise",
      year: 2025,
  ...
  # Task 3.5: Test cross-device scenario with habit updates
  test "cross-device cache invalidation when habit is updated on remote device" do
    habit = Habit.create!(
      user: @user,
      name: "Morning Exercise",
      year: 2025,
    ...
    # Device 1: Get initial cache state (simulates mobile app or tab)
    get habit_entries_month_path(year: 2025, month: 10)
    device1_etag = response.headers["ETag"]
    device1_last_modified = response.headers["Last-Modified"]
    assert_response :success

    # Device 1: Subsequent request with cache headers returns 304
    get habit_entries_month_path(year: 2025, month: 10), headers: {
      "If-None-Match" => device1_etag,
      "If-Modified-Since" => device1_last_modified
    }
    assert_response :not_modified

    # Device 2: Update habit data (simulates remote device making changes)
    travel 1.minute
    habit.update!(name: "Evening Exercise")

    # Device 2: Make a fresh request and get new cache headers
    get habit_entries_month_path(year: 2025, month: 10)
    device2_etag = response.headers["ETag"]
    device2_last_modified = response.headers["Last-Modified"]
    assert_response :success
    assert_not_equal device1_etag, device2_etag, "Device 2 should get new ETag after remote update"

    # Device 1: Request with old cache headers should now return fresh data
    get habit_entries_month_path(year: 2025, month: 10), headers: {
      "If-None-Match" => device1_etag,
      "If-Modified-Since" => device1_last_modified
    }
    assert_response :success, "Device 1 should get fresh data when cache is stale"
    assert_equal device2_etag, response.headers["ETag"], "Device 1 should now have same ETag as Device 2"
  end

  test "cross-device cache invalidation when habit entry is updated on remote device" do
    habit = Habit.create!(
      user: @user,
      name: "Morning Exercise",
      year: 2025,
    ...
    # Device 1: Get initial cache state
    get habit_entries_month_path(year: 2025, month: 10)
    device1_etag = response.headers["ETag"]
    device1_last_modified = response.headers["Last-Modified"]
    assert_response :success

    # Device 1: Verify cache hit works
    get habit_entries_month_path(year: 2025, month: 10), headers: {
      "If-None-Match" => device1_etag,
      "If-Modified-Since" => device1_last_modified
    }
    ...
    # Device 2: Update habit entry data via controller (simulates mobile app action)
    travel 1.minute
    patch habit_entry_path(entry), params: { habit_entry: { completed: true } }
    assert_response :no_content

    # Device 2: Get fresh data with new cache headers
    get habit_entries_month_path(year: 2025, month: 10)
    device2_etag = response.headers["ETag"]
    device2_last_modified = response.headers["Last-Modified"]
    assert_response :success
    assert_not_equal device1_etag, device2_etag, "Device 2 should get new ETag after entry update"

    # Device 1: Request with stale cache headers should get fresh data
    get habit_entries_month_path(year: 2025, month: 10), headers: {
      "If-None-Match" => device1_etag,
      "If-Modified-Since" => device1_last_modified
    }
    assert_response :success, "Device 1 should get fresh data when habit entry was updated remotely"
    assert_equal device2_etag, response.headers["ETag"], "Device 1 should now have same ETag as Device 2"
  end

  test "cross-device scenario with multiple habit changes shows proper cache invalidation sequence" do
    habit1 = Habit.create!(
      user: @user,
      name: "Morning Exercise",
      year: 2025,
    ...
    entry1 = HabitEntry.create!(habit: habit1, day: 1, completed: false)
    entry2 = HabitEntry.create!(habit: habit2, day: 1, completed: false)

    # Device 1: Initial cache state
    get habit_entries_month_path(year: 2025, month: 10)
    device1_etag_v1 = response.headers["ETag"]
    device1_last_modified_v1 = response.headers["Last-Modified"]

    # Device 2: Makes multiple changes sequentially
    travel 1.minute
    patch habit_entry_path(entry1), params: { habit_entry: { completed: true } }

    # Device 2: Check cache after first change
    get habit_entries_month_path(year: 2025, month: 10)
    device2_etag_v2 = response.headers["ETag"]
    device2_last_modified_v2 = response.headers["Last-Modified"]
    assert_not_equal device1_etag_v1, device2_etag_v2

    # Device 2: Makes second change
    travel 1.minute
    habit1.update!(name: "Evening Exercise")

    # Device 2: Check cache after second change
    get habit_entries_month_path(year: 2025, month: 10)
    device2_etag_v3 = response.headers["ETag"]
    device2_last_modified_v3 = response.headers["Last-Modified"]
    assert_not_equal device2_etag_v2, device2_etag_v3

    # Device 1: All previous cache versions should now be stale
    get habit_entries_month_path(year: 2025, month: 10), headers: {
      "If-None-Match" => device1_etag_v1,
      "If-Modified-Since" => device1_last_modified_v1
    }
    assert_response :success, "Device 1 v1 cache should be stale"

    get habit_entries_month_path(year: 2025, month: 10), headers: {
      "If-None-Match" => device2_etag_v2,
      "If-Modified-Since" => device2_last_modified_v2
    }
    assert_response :success, "Device 2 v2 cache should be stale"

    # Device 1: Should now have latest cache version
    assert_equal device2_etag_v3, response.headers["ETag"], "Device 1 should have latest ETag"
  end
end
